...
func (u *UserService) SendRegisterMail(ctx context.Context, user *User) error {
	...
	m.SetHeader("Subject", "感谢您注册我们的邮箱")
	link := fmt.Sprintf("您注册的token是： %v", user.Token)
...
func (u *UserService) LoginWithAddress(ctx context.Context, w *Wallet) (*Wallet, error) {
	return nil, nil
}
